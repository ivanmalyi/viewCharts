    ...
    public function charts(string $nameLine, int $id, Request $request)
    {
        try {
            ...
            if ($startTime == null or $endTime == null) {
                $startTime = \DateTime::createFromFormat('d-m-y H:i:s', date('d-m-y H:i:s', time() - 3600))->format('d-m-y H:i:s');
                $endTime = \DateTime::createFromFormat('d-m-y H:i:s', date('d-m-y H:i:s', time()))->format('d-m-y H:i:s');
        } else {
